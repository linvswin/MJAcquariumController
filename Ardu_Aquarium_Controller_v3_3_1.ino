...
int anno;
byte datotempo;
byte giornoimp, meseimp, oraimp, minutiimp, secondiimp;
int annoimp, annoold;
boolean Home;
...
byte OraOnPrec, OreTotPrec, MinOnPrec, MinTotPrec, OraOffPrec, MinOffPrec, OreFadPrec, MinFadPrec, OreLuceMax, MinLuceMax; // Variabili per controlli variazione
byte OreTot, MinTot, nrlinea;
int Fadeinmin, ITinmin, Offinmin, Oninmin, FAinmin;       // Variabili per calcolo inizio alba e tramonto
byte DatoFotoperiodo, TipoFotoperiodo, acquisizionedatifotoperiodo, Titoloimpostazionefotoperiodo, Parteimpostazione, LimitecaseInf, LimitecaseSup;
int Lucegiorno;
...
char* VociMenuluci[] =	{	"    IMPOSTA LUCI",       // Intestazione e voci del menu luci
                          "Fotoperiodo L1 ",
                          "Fotoperiodo L2 ",
                          "Fotoperiodo L3 ",
                          "Funz/LMax Linee",
...
boolean avvio, Impostazioni, Luci;
boolean stampato;
byte funzionamento;
...
//LiquidCrystal_I2C lcd(0x38, 20, 4); // Imposto l'indirizzo del display a 0x38 e dichiaro 20 colonne e 4 righe
...
}; //  vettore che contiene i dati per creare
...
String MJAcquariumCOntroller::getDate() {
  String txt = F("");
  txt += printDigit(this->now.Day()) + TXT_BARRA;
...
void setup()
{
  ...
  //lcd.init();                        // inizializzazione del display
  lcd.begin(20, 4);
